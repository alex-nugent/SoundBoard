...
enum class Ev : uint8_t {
  ...
  Tick1s, SetupRequest, JobProgress, Fault, PModeTrigger
};
...
struct Event {
  ...
  uint32_t t;
  union {
    struct { uint8_t pos, ch; int16_t delta; uint16_t pressId; uint16_t heldMs; } pad;   // delta in 0.1 %; heldMs on PadUp; PModeTrigger: pos only
